...
function FriendlyChat() {
  ...
  this.messageInput.addEventListener('keyup', this.updateObserveMessage.bind(this));
...
FriendlyChat.prototype.updateObserveMessage = function(e) {
  if (this.messageInput.value && this.checkSignedInWithMessage()) {
    ...
    if (this.currentMessageId == -1) {
      var postRef = this.observeMessagesRef.push({
          name: currentUser.displayName,
          text: this.messageInput.value,
          photoUrl: currentUser.photoURL || '/images/profile_placeholder.png'
      });
      ...
      var obj = {}
      var objKey = `${this.currentMessageId}/text`;
      obj[objKey] = this.messageInput.value;
      this.observeMessagesRef.update(obj);
    }
  }
}
